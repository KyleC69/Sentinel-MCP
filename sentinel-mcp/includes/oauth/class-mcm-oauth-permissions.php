...
if ( ! class_exists( 'MCPCOMAL_OAuth_Permissions' ) ) {

	/**
	 * Pre-execution gates: per-client allowlist and rate limiting.
	 */
	class MCPCOMAL_OAuth_Permissions {
		...
		public static function gate( $args, $tool_name, $mcp_tool, $server ) {
			$client_id = MCPCOMAL_OAuth_Interceptor::get_current_client_id();
			...
			if ( ! self::is_allowed( $client_id, (string) $tool_name ) ) {
				...
				return new WP_Error(
					...
					sprintf(
						...
						__( 'This OAuth client is not authorized to call ability "%s".', 'mcp-content-manager-lite' ),
						(string) $tool_name
					),
					array( 'status' => 403 )
				);
			...
			if ( ! $rate['ok'] ) {
				...
				return new WP_Error(
					'rate_limit_exceeded',
					__( 'Rate limit exceeded.', 'mcp-content-manager-lite' ),
					array(
						'status'              => 429,
