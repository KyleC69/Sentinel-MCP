...
if ( ! class_exists( 'MCPCOMAL_OAuth_Authorize' ) ) {

	/**
	 * Authorization endpoint handler for the OAuth 2.1 subsystem.
	 */
	class MCPCOMAL_OAuth_Authorize {
		...
		public static function handle_get( WP_REST_Request $request ) {
			...
			$client = MCPCOMAL_OAuth_DB::get_client_by_id( $params['client_id'] );
			if ( ! $client ) {
				return new WP_Error(
					'invalid_client',
					'Unknown client_id.',
			...
			if ( ! is_user_logged_in() ) {
				$current_url = rest_url( 'mcpcomal-auth/v1/authorize' ) . '?' . http_build_query( $params );
		...
		public static function handle_post( WP_REST_Request $request ) {
			...
			// Verify our custom nonce (named mcpcomal_oauth_nonce to avoid conflict
			// with the REST API's reserved _wpnonce parameter).
			$nonce = sanitize_text_field( $request->get_param( 'mcpcomal_oauth_nonce' ) ?? '' );
			if ( ! wp_verify_nonce( $nonce, 'mcpcomal_oauth_authorize' ) ) {
				return new WP_Error(
					'invalid_nonce',
			...
			$client = MCPCOMAL_OAuth_DB::get_client_by_id( $client_id );
			...
			$code = MCPCOMAL_OAuth_DB::insert_code(
				array(
					'client_id'             => $client_id,
		...
		private static function render_authorize_page( array $client, array $params ): void {
			...
			$nonce        = wp_create_nonce( 'mcpcomal_oauth_authorize' );
			$form_action  = rest_url( 'mcpcomal-auth/v1/authorize' );

			$scope_labels = array(
				'mcp:tools' => __( 'Use MCP tools to manage content', 'mcp-content-manager-lite' ),
				'mcp:read'  => __( 'Read content from your site', 'mcp-content-manager-lite' ),
				'mcp:write' => __( 'Create and modify content on your site', 'mcp-content-manager-lite' ),
			);
...
<head>
	...
	<title>
			...
			echo esc_html( sprintf( __( 'Authorize %s', 'mcp-content-manager-lite' ), $client['client_name'] ) . ' — ' . $site_name );
			?>
	</title>
			<?php
			wp_register_style(
				'mcpcomal-oauth-authorize',
				plugins_url( 'assets/css/oauth-authorize.css', dirname( __DIR__ ) ),
				array(),
				MCPCOMAL_VERSION
			);
			wp_print_styles( 'mcpcomal-oauth-authorize' );
			?>
</head>
<body>
	<div class="mcpcomal-oauth-card">
		<div class="mcpcomal-oauth-header">
			<?php if ( $site_icon ) : ?>
			...
			<p>
				...
				printf(
					/* translators: %s: client application name */
					esc_html__( 'The application %s is requesting access to your site.', 'mcp-content-manager-lite' ),
		...
		<div class="mcpcomal-oauth-scopes">
			<h3><?php esc_html_e( 'Requested permissions', 'mcp-content-manager-lite' ); ?></h3>
			<ul>
		...
		<p class="mcpcomal-oauth-user">
				<?php
				printf(
				/* translators: %s: current user display name */
					esc_html__( 'Logged in as %s', 'mcp-content-manager-lite' ),
		...
		<form method="post" action="<?php echo esc_url( $form_action ); ?>">
			<input type="hidden" name="mcpcomal_oauth_nonce" value="<?php echo esc_attr( $nonce ); ?>" />
			...
			<div class="mcpcomal-oauth-actions">
				<button type="submit" name="action" value="deny" class="mcpcomal-oauth-btn-deny">
					<?php esc_html_e( 'Deny', 'mcp-content-manager-lite' ); ?>
				</button>
				<button type="submit" name="action" value="authorize" class="mcpcomal-oauth-btn-authorize">
					<?php esc_html_e( 'Authorize', 'mcp-content-manager-lite' ); ?>
				</button>
			</div>
		</form>
	</div>
