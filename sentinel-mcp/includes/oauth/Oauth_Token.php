...
class OAuth_Token
{
	...
	public static function handle(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
	{
		...
		OAuth_DB::cleanup_expired_codes();
	...
	private static function handle_authorization_code(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
	{
		...
		$code_row = OAuth_DB::get_code($code);
		if (! $code_row) {
		...
		if (1 === (int) $code_row['used']) {
			OAuth_DB::revoke_all_for_client($code_row['client_id']);
			return self::oauth_error('invalid_grant', $grant_error);
		}
		...
		OAuth_DB::mark_code_used($code);

		$resolved_code_client_id = OAuth_DB::get_client_by_id_or_name($code_row['client_id'])['client_id'] ?? $code_row['client_id'];
		$resolved_request_client_id = OAuth_DB::get_client_by_id_or_name($client_id)['client_id'] ?? $client_id;
		...
		// 8. Generate tokens.
		$token_data = OAuth_DB::insert_token(
			array(
				'client_id' => $client_id,
	...
	private static function handle_refresh_token(\WP_REST_Request $request): \WP_REST_Response|\WP_Error
	{
		...
		$refresh_hash = OAuth_DB::hash_token($refresh_token);
		...
		OAuth_DB::revoke_token_by_refresh_hash($refresh_hash);

		// Issue new token pair.
		$new_tokens = OAuth_DB::insert_token(
			array(
				'client_id' => $client_id,
	...
	public static function handle_revoke(\WP_REST_Request $request): \WP_REST_Response
	{
		...
		$hash = OAuth_DB::hash_token($token);

		if ('refresh_token' === $token_hint) {
			OAuth_DB::revoke_token_by_refresh_hash($hash);
		} else {
			// Try access token first, then refresh.
			$revoked = OAuth_DB::revoke_token_by_access_hash($hash);
			if (! $revoked) {
				OAuth_DB::revoke_token_by_refresh_hash($hash);
			}
		}
