 ...
 */
class Admin_Tab_Connect extends Admin_Tab
{
	...
	public function render(): void
	{
		...
		$clients    = Config_Exporter::clients();
		if (! array_key_exists($selected, $clients)) {
			$selected = 'claude_desktop';
		}

		$config = Config_Exporter::for_client($selected);
?>
		<div class="card" style="max-width:1100px;margin-bottom:20px;padding:15px;">
			...
			<div style="background:#f0f6fc;border:1px solid #2271b1;border-radius:4px;padding:10px;margin-bottom:15px;">
				<code style="font-size:14px;word-break:break-all;user-select:all;"><?php echo esc_html(Config_Exporter::endpoint_url()); ?></code>
			</div>
