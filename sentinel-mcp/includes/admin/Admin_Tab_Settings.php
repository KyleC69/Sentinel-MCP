 ...
 *
 * Also handles its own POST action (SENTINEL_save_settings).
 */
class Admin_Tab_Settings extends Admin_Tab
{
	...
	{
		$debug_enabled = (bool) get_option('sentinel_debug_logging', false);
		$gemini_key    = (string) get_option('sentinel_gemini_api_key', '');
		$gemini_model  = (string) get_option('sentinel_gemini_model', Image_Generator::DEFAULT_MODEL);
		$key_masked    = '' === $gemini_key ? '' : str_repeat('•', max(0, strlen($gemini_key) - 4)) . substr($gemini_key, -4);
?>
		<form method="post">
			<?php wp_nonce_field('sentinel_save_settings'); ?>
				...
				<table class="form-table" style="margin:0;">
					<tr>
						...
						<td style="padding:8px 10px;">
							<label>
								<input type="checkbox" name="sentinel_debug_logging" value="1" <?php checked($debug_enabled); ?> />
				...
				<table class="form-table" style="margin:0;">
					<tr>
						...
						<td style="padding:8px 10px;">
							...
							<?php if ('' !== $gemini_key) : ?>
								<button type="submit" name="sentinel_clear_gemini_api_key" value="1" class="button button-link-delete" formnovalidate>
			...
			<p><button type="submit" name="sentinel_save_settings" value="1" class="button button-primary"><?php esc_html_e('Save Settings', 'mcp-sentinel'); ?></button></p>
		</form>
<?php
	}
	...
	{
		if (! isset($_POST['sentinel_save_settings'])) {
			return false;
		}

		check_admin_referer('sentinel_save_settings');

		// Explicit "Remove key" button takes precedence over the input field.
		if (! empty($_POST['sentinel_clear_gemini_api_key'])) {
			update_option('sentinel_gemini_api_key', '', false);
			$this->redirect_with_notice('settings', 'success', 'Gemini API key removed.');
		}

		update_option('sentinel_debug_logging', ! empty($_POST['sentinel_debug_logging']));

		if (isset($_POST['sentinel_gemini_api_key'])) {
			$key = trim(sanitize_text_field(wp_unslash((string) $_POST['sentinel_gemini_api_key'])));
			// Empty input = keep current key (so re-saving the form does not wipe it).
			if ('' !== $key) {
				update_option('sentinel_gemini_api_key', $key, false);
			}
		}
		if (isset($_POST['sentinel_gemini_model'])) {
			$model = sanitize_text_field(wp_unslash((string) $_POST['sentinel_gemini_model']));
			update_option('sentinel_gemini_model', '' === $model ? Image_Generator::DEFAULT_MODEL : $model, false);
		}
