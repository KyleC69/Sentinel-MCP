...
if (! class_exists('SentinelMCP\SENTINEL_Admin_Tab_Providers')) {

	/**
	 * Renders the Providers tab: lists available AI connectors and their status.
	 */
	class SENTINEL_Admin_Tab_Providers extends SENTINEL_Admin_Tab
	{
		...
		public function render(): void
		{
			...
			$providers     = $has_ai_client ? \WordPress\AiClient\AiClient::defaultRegistry()->wp_get_connectors() : [];
			?>
			<div class="card" style="max-width:900px;margin-bottom:20px;padding:15px;">
				...
				<?php elseif (empty($providers)) : ?>
					<p><?php esc_html_e('No AI providers are currently registered.', 'mcp-sentinel'); ?></p>
				<?php else : ?>
		 ...
		 * mcpcomal_get_connector_api_key_source() for source detection.
		 *
		...
		private function get_connector_status(string $id): array
		{
			if (! class_exists('\WordPress\AiClient\AiClient')) {
				return [
					'configured'    => false,
					'source'        => __('AI Client unavailable', 'mcp-sentinel'),
					'method'        => 'none',
					'source_detail' => '',
				];
			}
			...
			$source = mcpcomal_get_connector_api_key_source(
				$auth['setting_name']  ?? '',
				$auth['env_var_name']  ?? '',
				$auth['constant_name'] ?? ''
			);
